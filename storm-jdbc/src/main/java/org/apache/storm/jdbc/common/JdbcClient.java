...
public class JdbcClient {
    ...
    public void insertWithCache(String tableName, List<List<Column>> columnLists) {
        String query = constructInsertQuery(tableName, columnLists);
        try {
			executeInsertQueryWithCache(query, columnLists);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
    }
    public void setRowCache(int rowCache) {
    	this.rowCache = rowCache;
    }
    
    public int getRowCache() {
    ...
    public void executeInsertQueryWithCache(String query, List<List<Column>> columnLists) throws SQLException {
        if(this.rowCache < 0) {
        	throw new IllegalArgumentException("please make sure row cache larger than 0");
        }
        if(this.longConnection == null || this.longConnection.isClosed()) {
        	longConnection =connectionProvider.getConnection();
            boolean autoCommit = longConnection.getAutoCommit();
            if(autoCommit) {
            	longConnection.setAutoCommit(false);
            }
        }
        try {
            ...
            PreparedStatement preparedStatement = longConnection.prepareStatement(query);
            if(queryTimeoutSecs > 0) {
                preparedStatement.setQueryTimeout(queryTimeoutSecs);
            }
            ...
            int[] results = preparedStatement.executeBatch();
            if(Arrays.asList(results).contains(Statement.EXECUTE_FAILED)) {
            	longConnection.rollback();
                throw new RuntimeException("failed at least one sql statement in the batch, operation rolled back.");
            } else {
                try {
                	this.rowCount++;
                	if (this.rowCount >= this.rowCache) {
                		longConnection.commit();
                		LOG.debug("commit " + rowCount + " record");
                		rowCount = 0;
                	}
                } catch (SQLException e) {
                    throw new RuntimeException("Failed to commit insert query " + query, e);
    ...
    public void closeLongConnection() {
    	closeConnection(longConnection);
    }
    
    public void LongConnectionCommit() {
    	try {
			this.longConnection.commit();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
